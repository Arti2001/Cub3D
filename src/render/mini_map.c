...
/*   Updated: 2025/02/11 18:06:36 by amysiv           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_map2(t_root *data, int img[2], int flag)
{
	...
	if (flag == WALL)
		...
		color = 0x0;
	if (img[X] >= 0 && img[X] < MM_DIMENTION && \
		img[Y] >= 0 && img[Y] < MM_DIMENTION)
...
void	add_mini_map(t_root *data)
{
	...
	while (y < MM_DIMENTION)
	{
		...
		while (x < MM_DIMENTION)
		{
			...
			mm_coord[X] = x;
			mm_coord[Y] = y;
			if (map_x >= 0 && map_x < data->map.lenght \
				&& map_y >= 0 && map_y < data->map.height)
			{
				tile = data->map.map[map_y][map_x];
				if (tile == '1')
